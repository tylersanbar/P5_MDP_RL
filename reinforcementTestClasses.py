...
class Question8Test(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        studentSolution = moduleDict['P5_supplement'].question8()
...
class GridPolicyTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        # Function in module in P5_supplement that returns (discount, noise)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        if not hasattr(moduleDict['P5_supplement'], self.parameterFn):
            self.addMessage('Method not implemented: P5_supplement.%s' % (self.parameterFn,))
            return self.testFail(grades)

        result = getattr(moduleDict['P5_supplement'], self.parameterFn)()
        ...
        if self.question2:
            ...
            try:
                ...
                self.addMessage('Did not return a (discount, noise) pair; instead P5_supplement.%s returned: %s' % (self.parameterFn, result))
                return self.testFail(grades)
            ...
            try:
                ...
                self.addMessage('Did not return a (discount, noise, living reward) triple; instead P5_supplement.%s returned: %s' % (self.parameterFn, result))
                return self.testFail(grades)
